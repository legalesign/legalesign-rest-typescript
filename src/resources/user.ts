...
export class User extends APIResource {
  ...
  retrieve(userID: string, options?: RequestOptions): APIPromise<UserRetrieveResponse> {
    return this._client.get(path`/user/${userID}/`, options);
  }
   ...
   * ```
   */
  update(userID: string, body: UserUpdateParams, options?: RequestOptions): APIPromise<void> {
    return this._client.patch(path`/user/${userID}/`, {
      body,
      ...options,
      headers: buildHeaders([{ Accept: '*/*' }, options?.headers]),
    });
